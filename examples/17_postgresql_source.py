"""Read a PostgreSQL repeatable-read snapshot with the native source connector.
...
def build_project(directory: Path) -> ProjectDocument:
    graph = PipelineBuilder("postgresql-source").expression(
...
async def run(directory: Path, *, timeout: float = 60) -> None:
    ...
    try:
        ...
        if totals != [20.0, 60.0]:
            raise RuntimeError(f"unexpected postgresql totals: {totals}")
        print("postgresql -> Parquet:", totals)
...
def main() -> None:
    ...
    if not {"file", "postgresql"} <= available:
        raise SystemExit(
            "Build a wheel with connector-postgresql and connector-file; "
            "see examples/README.md."
        )
    if not os.environ.get("CALC_FLOW_PG_URL"):
        raise SystemExit(
            "Set CALC_FLOW_PG_URL after preparing the sample service; "
            "see examples/README.md."
        )
    with TemporaryDirectory(prefix="calc-flow-postgresql-source-") as directory:
