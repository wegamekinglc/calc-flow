"""Write calculated order totals through the native kafka sink.

Prepare the empty demo destination in docs/connectors/kafka.md first.
...
def build_project(directory: Path) -> cf.ProjectDocument:
    ...
    sink = {
        ...
        "options": {
            ...
            "topic": "calc-flow-example-totals",
            "ledger_topic": "calc-flow-example-totals-ledger",
...
async def run(directory: Path, *, timeout: float = 60) -> None:
    ...
    try:
        ...
        if rows != 2:
            raise RuntimeError(f"unexpected kafka delivered rows: {rows}")
        if status["delivery"]["output"]["effective"] != "exactly_once":
            raise RuntimeError("unexpected kafka delivery guarantee")
        print(
            "Parquet -> kafka: wrote 2 rows; "
...
def main() -> None:
    ...
    if not {"file", "kafka"} <= available:
        raise SystemExit(
            "Build connector-file and connector-kafka; see examples/README.md."
        )
    with TemporaryDirectory(prefix="calc-flow-kafka-sink-") as directory:
