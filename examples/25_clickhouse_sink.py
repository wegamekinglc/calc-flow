"""Write calculated order totals through the native clickhouse sink.

Prepare the empty demo destination in docs/connectors/clickhouse.md first.
...
def build_project(directory: Path) -> cf.ProjectDocument:
    ...
    program = cf.Program(
        "clickhouse-sink", outputs={"totals": orders.pipe(order_totals)}
    )
    sink = {
        "binding": "output",
        "connector": {
        ...
        "secrets": {"url": {"resolver": "environment", "key": "CALC_FLOW_CH_URL"}},
        "options": {
...
async def run(directory: Path, *, timeout: float = 60) -> None:
    ...
    try:
        ...
        if rows != 2:
            raise RuntimeError(f"unexpected clickhouse delivered rows: {rows}")
        if status["delivery"]["output"]["effective"] != "at_least_once":
            raise RuntimeError("unexpected clickhouse delivery guarantee")
        print(
            "Parquet -> clickhouse: wrote 2 rows; "
...
def main() -> None:
    ...
    if not {"file", "clickhouse"} <= available:
        raise SystemExit(
            "Build connector-file and connector-clickhouse; see examples/README.md."
        )
    if not os.environ.get("CALC_FLOW_CH_URL"):
        raise SystemExit("Set CALC_FLOW_CH_URL; see docs/connectors/clickhouse.md.")
    with TemporaryDirectory(prefix="calc-flow-clickhouse-sink-") as directory:
