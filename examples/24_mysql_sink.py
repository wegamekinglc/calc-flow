"""Write calculated order totals through the native mysql sink.

Prepare the empty demo destination in docs/connectors/mysql.md first.
...
def build_project(directory: Path) -> cf.ProjectDocument:
    ...
    program = cf.Program("mysql-sink", outputs={"totals": orders.pipe(order_totals)})
    sink = {
        "binding": "output",
        "connector": {
...
async def run(directory: Path, *, timeout: float = 60) -> None:
    ...
    try:
        ...
        if rows != 2:
            raise RuntimeError(f"unexpected mysql delivered rows: {rows}")
        if status["delivery"]["output"]["effective"] != "at_least_once":
            raise RuntimeError("unexpected mysql delivery guarantee")
        print(
            "Parquet -> mysql: wrote 2 rows; "
...
def main() -> None:
    ...
    if not {"file", "mysql"} <= available:
        raise SystemExit(
            "Build connector-file and connector-mysql; see examples/README.md."
        )
    if not os.environ.get("CALC_FLOW_MYSQL_URL"):
        raise SystemExit("Set CALC_FLOW_MYSQL_URL; see docs/connectors/mysql.md.")
    with TemporaryDirectory(prefix="calc-flow-mysql-sink-") as directory:
