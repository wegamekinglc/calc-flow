...
`WindowAggregateOperator`, and the two-input bounded event-time
`StreamJoinOperator`. A union's two or more input ports share one kind, one
required flag, and one exact schema, or are all schema-less.
