...
aggregate or SQL window is never silently made batch-local. A rolling
declaration over a `duration` frame and a `cs.winsorize` declaration — the
one cross-section primitive withheld in this release — fail with
`unsupported_type` rooted at the output feature. Array outputs
