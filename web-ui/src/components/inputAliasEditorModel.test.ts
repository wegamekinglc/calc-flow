...
describe('SQL input aliases', () => {
  ...
  it('renames the operator alias, schema port, and matching incoming edge', () => {
    ...
    expect(at(updated.graph.nodes).operator).toMatchObject({
      aliases: ['left', 'rhs'],
    });
    expect(at(at(updated.graph.nodes).input_ports, 1)).toEqual({
      name: 'rhs',
      kind: 'table',
      required: true,
      schema: [{ name: 'id', data_type: 'int64', nullable: false }],
    ...
    expect(updated.graph.edges.map((edge) => edge.target_port))
      .toEqual(['left', 'rhs']);
    expect(at(at(project.graph.nodes).input_ports, 1).name).toBe('right');
    expect(at(project.graph.edges, 1).target_port).toBe('right');
  });
  ...
  it('removes only the selected alias, port, and incoming edge', () => {
    ...
    expect(at(updated.graph.nodes).operator).toMatchObject({ aliases: ['left'] });
    expect(at(updated.graph.nodes).input_ports.map((port) => port.name))
      .toEqual(['left']);
  ...
  it('adds a derived alias without materializing explicit ports', () => {
    ...
    expect(at(updated.graph.nodes).input_ports).toEqual([]);
  });
