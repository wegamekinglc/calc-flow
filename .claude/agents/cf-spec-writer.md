...
needed) using this template, trimming sections that are unnecessary for the task under the shared
delivery policy in `.codex/agents/README.md`:
...
- [ ] <named targeted local checks pass; full regression/coverage gates assigned to CI>
...
"the named targeted checks pass, with full regression assigned to CI" is. Pending
CI can be recorded at handoff; it does not establish merge readiness. See `AGENTS.md`
Verification for local-full-test exceptions.
